...
def load_dataset(
    ...
    print(
        f"Rows parsed: {len(dataframe)}"
    )
    print(
        f"Columns parsed: {len(dataframe.columns)}"
    )
    print(
        f"Target table: {raw_schema}.{raw_table}"
    )
    ...
    print(
        f"Loaded {raw_schema}.{raw_table}"
    )

    # Keep this as the final output line so Streamlit can parse it.
    print(
        json.dumps(
            summary
        )
